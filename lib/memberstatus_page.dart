...
class MemberStatusPageState extends State<MemberStatusPage> {
  ...
  void initState() {
    super.initState();
    _membersFuture = EventRepository().getEventMembers(widget.eventId); // 最初に一度だけデータを取得しにいく
  }

  // ステータスボタン定義
  Widget _statusButton(String label, IconData icon, Color iconColor, int? filterValue) {
    return Padding(
      padding: const EdgeInsets.all(5),
      child: SizedBox(
        height: 40,
        width: 126,
        child: ElevatedButton.icon( // 押したボタンのステータス状態の人を表示
          onPressed: () {
            setState(() {
    ...
    return  FutureBuilder<List<dynamic>> ( // 作業終わるまで置き換えておく画面作成
      ...
      builder: (context, snapshot) {  // 状況(snapshot)に合わせて作る画面作成
        ...
        final List<dynamic> filterMembers = selectedFilter == null
          ...
          : eventMembers.where((m) => m['status'] == selectedFilter).toList();  // 選択されたフィルターと同じ値のstatusの人をリスト化
        
        final int all = eventMembers.length;  // イベントメンバー人数
        final int arrived = eventMembers.where((m) => m['status'] == 4).length; // イベントメンバーの内到着済み人数
        ...
        return Column(
          children: [
            ...
            Padding(
              ...
              child: SizedBox(
                ...
                child: ElevatedButton.icon(
                  ...
                  icon: const Icon(Icons.qr_code_2, color: Colors.black, size: 25),
                  label: const Text(
                    'SHOW QR CODE',
                    style: TextStyle(
                      color: Colors.black,
            ...
            SingleChildScrollView(
              ...
              child: Row(
                children: [
                  _statusButton('ALL MEMBERS', Icons.filter_list, Colors.deepOrangeAccent, null),
                  _statusButton('0: SLEEPING', Icons.circle, Colors.grey, 0),
                  _statusButton('1: AWAKE', Icons.circle, Colors.greenAccent, 1),
                  _statusButton('2: OVERSLEPT', Icons.circle, Colors.pinkAccent, 2),
                  _statusButton('3: MOVING', Icons.circle, Colors.lightBlueAccent, 3),
                  _statusButton('4: ARRIVED', Icons.circle, Colors.orangeAccent, 4),
                ],
              ),
            ),
            ...
            Expanded(
              ...
              ? const Center(child: Text('No members match this status.'))  // イベントメンバーがいないの時
              : ListView.builder( // イベントメンバーがいる時
                ...
                itemBuilder: (context, index) {
                  ...
                  final member = filterMembers[index] as Map<String, dynamic>? ?? {};
                  ...
                  return GestureDetector(
                    onTap: () async {
                      ...
                      try {
                        final report = await EventRepository().getMemberReport(widget.eventId, memberId);
                        
                        if (!mounted) return;

                        if (report != null) {
                          Navigator.push( // ポストページに移動
                            context,
                            MaterialPageRoute(
                              builder: (context) => MemberPostPage(member: member, report: report)
                            ),
                          );
                          debugPrint('ポスト画面へ移動');
                        } else {
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(content: Text('There are no posts yet.'))
                          );
                        }
                      } catch (e) {
                        debugPrint('$e');
                        ...
                        children: [
                          CircleAvatar(
                            ...
                            backgroundImage: (member['avatar_url'] != null && member['avatar_url'] != "")
                                ? NetworkImage(member['avatar_url'])  // アイコンを写真のurlにする
                                : null,
                            child: (member['avatar_url'] == null || member['avatar_url'] == "")
                                ? const Icon(Icons.person, color: Colors.white) // 代替アイコンを使う
                                : null,
                          ),
